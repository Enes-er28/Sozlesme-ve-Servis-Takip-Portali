...
if($hasFatura) $html .= '<th width="50" align="center">KDV</th>';
...
foreach($detay['hareketler'] as $h) {
    ...
    // Eğer faturalı ise KDV uygula
    if(!empty($h['fatura']) && isset($h['kdv'])) {
        $tutar *= (1 + $h['kdv'] / 100);
    }
    ...
    if($hasFatura) {
        $kdvDegeri = !empty($h['fatura']) ? ($h['kdv'] ?? 0) : '';
        $html .= '<td width="50" align="center">'.($kdvDegeri !== '' ? '%'.$kdvDegeri : '').'</td>';
    }
...
$altX = $xStart + 104.1; // sağa yapışık (istersen manuel değiştirebilirsin)
$pdf->SetXY($altX, $altY);
...
if($toplamFiyat > 0) {
    if($hasFatura) {
        $kdv = $toplamTutar - $toplamFiyat;

        $htmlAlt .= '<tr style="background-color:#ffffff; font-weight:bold;">
            <td width="120" align="right">Brüt Toplam</td>
            <td width="70" align="right">'.number_format($toplamFiyat, 2, ',', '.').' </td>
        </tr>';

        $htmlAlt .= '<tr style="background-color:#ffffff; font-weight:bold;">
            <td width="120" align="right">KDV</td>
            <td width="70" align="right">'.number_format($kdv, 2, ',', '.').' </td>
        </tr>';
        ...
        $htmlAlt .= '<tr style="background-color:#ffffff; font-weight:bold;">
            <td width="70" align="right">Brüt Toplam</td>
            <td width="70" align="right">'.number_format($toplamFiyat, 2, ',', '.').' </td>
        </tr>';
    }
}
...
$pdf->Image('resimler/Alt_ETA2.jpeg', 30, 249, 155); // X=15mm, Y=10mm, Genişlik=50mm
